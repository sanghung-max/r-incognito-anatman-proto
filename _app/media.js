function renderNodeMedia(node) {
    ...
    node.pdf_manifest.forEach((item, idx) => {
      pdfHTML += `
        <div class="pdf-item-card" style="border: 1px solid #333; padding: 12px; border-radius: 8px;">
          ${item.thumbnail ? `<img src="${item.thumbnail}" loading="lazy" decoding="async" style="max-width: 200px; display: block; margin-bottom: 8px;" />` : ''}
  ...
  if (node.img_color) {
    container.innerHTML = `<img src="${node.img_color}" loading="lazy" decoding="async" style="max-width:100%; height:auto;" />`;
  }
}