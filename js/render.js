...
let contentHtml = "";
  ...
  `;
  // NEW: Re-apply the current font size variable after rendering tab HTML
  if (typeof applyFontSize === 'function') {
    applyFontSize(window.appState?.drawerFontSize || 16);
  }
...
function renderView(nodeId) {
 ...
 //   .filter(child => child && child.ui_render !== false);
	.filter(child => {
    if (!child || child.ui_render === false) return false;
    // If on root page, filter against user's Board Visibility preferences
    if (nodeId === "0-0__0000" && typeof isBoardVisible === "function") {
      return isBoardVisible(child.id);
    }
    return true;
  });
  ...
  // ALWAYS display multi-stage drawer width controls
  if (drawerControls) drawerControls.style.display = "flex";

  if (!isLeafNode) {
    // --- NON-LEAF (BOARD/FOLDER) VIEWS ---
    if (sidebar && !sidebar.classList.contains("side-nav-overlay-peek")) {
      sidebar.className = "side-nav-hidden";
    }
    ...
    // Render metadata content inside drawer if drawer is toggled open
    ...
    if (sidebar) {
      sidebar.classList.remove("side-nav-overlay-peek", "side-nav-hidden");
      if (!sidebar.className.includes("side-nav-")) {
        sidebar.classList.add("side-nav-standard");
      }
    }
